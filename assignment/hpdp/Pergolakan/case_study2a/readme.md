...
As you can see, the correlation between our columns are huge, one of the SweetViz's cons is that it can give Overwhelming Output to the users hence we try to narrow it down by creating a report on only one of the 3 datasets which is, the death dataset.

On the implementation of SweetViz, we can change the df table to death table and assigned new value to the report generated
```
report2 = sv.analyze(death) #generate SweetViz report on the death data
```

Then, we can explore pattern and demographic info for death table from SweetViz tool, we run
```
report2.show_html('SweetViz_Report2.html') #show report on death data
```

### Repeat the steps by downloading a SweetViz_Report2.html at the file tab or you can just download from <a href="SweetViz_Report2.html" download="SweetViz Report death">here</a>

The correlation on death data generated by SweetViz now is far better to see and analyze by us
<p align="center">
    <img src="https://raw.githubusercontent.com/drshahizan/Python_EDA/main/assignment/hpdp/Pergolakan/case_study2a/SweetViz4.png" width=1500>
</p>
